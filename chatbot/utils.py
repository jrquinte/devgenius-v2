...
def read_agent_response(event_stream):
    """
    Función con debugging completo para entender el formato de respuesta
    """
    ask_user = False
    agent_answer = ""
    
    print("=" * 60)
    print("🔍 DEBUG: Starting read_agent_response")
    print(f"📊 Event stream type: {type(event_stream)}")
    print(f"📊 Event stream repr: {repr(event_stream)}")
    
    # Si es una lista, mostrar información básica
    if isinstance(event_stream, list):
        print(f"📋 Event stream is a list with {len(event_stream)} items")
        for i, item in enumerate(event_stream[:3]):  # Mostrar solo los primeros 3
            print(f"📋 Item {i}: {type(item)} - {repr(item)[:200]}...")
    ...
    try:
        ...
        for event in event_stream:
            ...
            print(f"📊 Event content: {repr(event)[:300]}...")
            
            if 'chunk' in event:
                print("✅ Found 'chunk' in event")
                chunk_data = event['chunk']
                print(f"📦 Chunk type: {type(chunk_data)}")
                print(f"📦 Chunk keys: {list(chunk_data.keys()) if isinstance(chunk_data, dict) else 'Not a dict'}")
                
                if 'bytes' in chunk_data:
                    print("✅ Found 'bytes' in chunk")
                    data = chunk_data['bytes']
                    print(f"📊 Bytes data type: {type(data)}")
                    print(f"📊 Bytes data length: {len(data) if hasattr(data, '__len__') else 'No length'}")
                    
                    if isinstance(data, bytes):
                        decoded = data.decode('utf8')
                        print(f"📝 Decoded text: {repr(decoded[:200])}...")
                        agent_answer += decoded
                    else:
                        str_data = str(data)
                        print(f"📝 String data: {repr(str_data[:200])}...")
                        agent_answer += str_data
                else:
                    print("❌ No 'bytes' found in chunk")
                    print(f"📦 Available chunk keys: {list(chunk_data.keys()) if isinstance(chunk_data, dict) else 'N/A'}")
                    
            elif 'trace' in event:
                print("✅ Found 'trace' in event")
                trace_data = event['trace']
                print(f"🔍 Trace type: {type(trace_data)}")
                print(f"🔍 Trace keys: {list(trace_data.keys()) if isinstance(trace_data, dict) else 'Not a dict'}")
                print(f"🔍 Trace content: {repr(trace_data)[:300]}...")
                
                try:
                    if isinstance(trace_data, dict) and 'trace' in trace_data:
                        inner_trace = trace_data['trace']
                        print(f"🔍 Inner trace type: {type(inner_trace)}")
                        print(f"🔍 Inner trace keys: {list(inner_trace.keys()) if isinstance(inner_trace, dict) else 'Not a dict'}")
                        
                        if isinstance(inner_trace, dict) and 'orchestrationTrace' in inner_trace:
                            orch_trace = inner_trace['orchestrationTrace']
                            print(f"🎼 Orchestration trace type: {type(orch_trace)}")
                            print(f"🎼 Orchestration trace content: {repr(orch_trace)[:300]}...")
                            print(f"orchestration trace = {orch_trace}")
                            
                            if isinstance(orch_trace, dict) and 'observation' in orch_trace:
                                observation = orch_trace['observation']
                                print(f"👁️ Observation type: {type(observation)}")
                                print(f"👁️ Observation content: {repr(observation)}")
                                
                                if isinstance(observation, dict) and observation.get('type') == "ASK_USER":
                                    print("🙋 ASK_USER detected - setting ask_user = True")
                                    ask_user = True
                                else:
                                    print("🙋 No ASK_USER - setting ask_user = False")
                                    ask_user = False
                            else:
                                print("❌ No 'observation' in orchestrationTrace")
                                ask_user = False
                        else:
                            print("❌ No 'orchestrationTrace' in inner trace")
                            print(f"🔍 Available inner trace keys: {list(inner_trace.keys()) if isinstance(inner_trace, dict) else 'N/A'}")
                    else:
                        print("❌ No 'trace' in trace data or trace data is not dict")
                        print(f"🔍 Available trace keys: {list(trace_data.keys()) if isinstance(trace_data, dict) else 'N/A'}")
                        
                except Exception as trace_error:
                    print(f"❌ Error processing trace: {str(trace_error)}")
                    print(f"❌ Trace error type: {type(trace_error)}")
                    import traceback
                    print(f"❌ Full traceback: {traceback.format_exc()}")
                    
            else:
                print("❌ Unexpected event structure")
                print(f"📊 Available event keys: {list(event.keys()) if isinstance(event, dict) else 'Not a dict'}")
                print(f"📊 Full event: {repr(event)}")
                raise ValueError(f"Unexpected event: {event}")
                
        print(f"\n✅ Processed {event_count} events total")
        print(f"📝 Final agent_answer length: {len(agent_answer)}")
        print(f"📝 Final agent_answer preview: {repr(agent_answer[:200])}...")
        print(f"🙋 Final ask_user: {ask_user}")
...
def collect_feedback(uuid, response, use_case, bedrock_model_name):
    """
    Collect feedback with error handling
    """
    import streamlit as st
    ...
    try:
        ...
        if selected is not None:
            text = st.text_input(
                f"fe-{uuid}", label_visibility="hidden",
                placeholder="[OPTIONAL] Please provide feedback explanation",
            )
            if text:
                try:
                    current_datetime = datetime.datetime.now(tz=datetime.timezone.utc)
                    ...
                    print(f"⚠️ Could not save feedback to DynamoDB: {str(e)}")
                    st.info("Feedback noted (but not saved to database)")
    except Exception as e:
        print(f"⚠️ Feedback collection error: {str(e)}")
        # Show basic feedback without DynamoDB
        selected = st.feedback("thumbs", key=f"s-{uuid}")
        if selected is not None:
            sentiment_mapping = ["👎", "👍"]
            st.markdown(f"Thanks for your feedback: {sentiment_mapping[selected]}")
...
def retrieve_environment_variables(key):
    """
    Retrieve environment variables directly instead of from JSON
    """
    value = os.getenv(key)
    if value is None:
...
def save_conversation(conversation_id, prompt, response):
    """
    Store conversation details in DynamoDB with error handling
    """
    try:
        CONVERSATION_TABLE_NAME = retrieve_environment_variables("CONVERSATION_TABLE_NAME")
        item = {
        ...
        dynamodb_resource.Table(CONVERSATION_TABLE_NAME).put_item(Item=item)
        print(f"✅ Conversation saved to DynamoDB")
    except Exception as e:
        print(f"⚠️ Could not save to DynamoDB: {str(e)}")
        print(f"📝 Conversation logged locally instead")
        # For testing purposes, just print the conversation
        print(f"Conversation ID: {conversation_id}")
        print(f"User: {prompt[:100]}..." if len(prompt) > 100 else f"User: {prompt}")
        print(f"Assistant: {response[:100]}..." if len(response) > 100 else f"Assistant: {response}")
        # Don't raise the error - just continue without saving to DynamoDB
